...
<div class="effort-selector relative">
  <button
    ...
    class="flex items-center gap-0.5 px-2 py-1.5 rounded-full text-sm transition-colors
      {value === 'STANDARD'
  ...
  >
    ...
    <Icon icon="mdi:chevron-up" class="w-3 h-3 transition-transform {menuOpen ? 'rotate-180' : ''}" />
  </button>

  {#if menuOpen}
    <div class="absolute bottom-full left-0 mb-1 bg-white border border-slate-200 rounded-lg shadow-lg py-1 min-w-35 z-50">
      {#each effortOptions as option}
        <button
          ...
          class="w-full flex items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-slate-50 transition-colors
            {value === option.value ? 'text-indigo-600' : 'text-slate-700'}"
        >
