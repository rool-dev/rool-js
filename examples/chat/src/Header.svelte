...
<header class="bg-white border-b border-slate-200 px-4 py-3">
  <div class="max-w-5xl mx-auto flex items-center gap-3">
    ...
    <div class="flex items-center gap-2 shrink-0">
      <div class="w-8 h-8 bg-linear-to-br from-indigo-400 to-blue-500 rounded-lg flex items-center justify-center">
        <Icon icon="mdi:chat-outline" class="w-4 h-4 text-white" />
      </div>
      <span class="font-semibold text-slate-800 hidden sm:block">Chat</span>
    </div>
    ...
    <div class="flex-1 min-w-0 relative" data-dropdown>
      ...
      {:else if showNewSpaceForm}
        <div class="flex items-center gap-2">
          <input
            type="text"
            class="flex-1 min-w-0 px-3 py-1.5 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none"
          ...
          />
          <button
            class="px-3 py-1.5 text-sm font-medium text-white bg-indigo-500 rounded-lg hover:bg-indigo-600 disabled:opacity-50 shrink-0"
            onclick={createSpace}
