...
<div class="flex-1 overflow-auto min-h-0 p-4 pb-24 space-y-3" bind:this={container}>
  {#if sorted.length === 0 && !messages.loading}
    <div class="flex flex-col items-center justify-center h-full text-center">
      <div class="w-16 h-16 bg-teal-100 dark:bg-teal-900/40 rounded-2xl flex items-center justify-center mb-6">
        <Icon icon="mdi:pound" class="w-8 h-8 text-teal-500 dark:text-teal-400" />
      </div>
      <h3 class="text-lg font-semibold text-slate-700 dark:text-neutral-100 mb-2"># {huddleName}</h3>
    ...
    {#each sorted as msg (msg.id)}
      ...
      <div class="flex items-start gap-2.5 {isOwn && !isAgent ? 'flex-row-reverse' : ''}">
        ...
        <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-semibold
          {isAgent ? 'bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-400' : isOwn ? 'bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-400' : 'bg-slate-200 dark:bg-neutral-700 text-slate-600 dark:text-neutral-300'}">
          {#if isAgent}
        ...
        <div class="max-w-[75%] min-w-0">
          ...
          <div class="rounded-2xl px-3.5 py-2 text-sm
            {isAgent
              ? 'bg-violet-50 dark:bg-violet-900/20 text-slate-700 dark:text-neutral-200 rounded-tl-md markdown-output'
              : isOwn
...
<style>
  ...
  .markdown-output :global(code) {
    @apply bg-violet-100/50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 px-1 py-0.5 rounded text-xs font-mono;
  }
  .markdown-output :global(pre) {
    @apply bg-slate-900 dark:bg-black text-slate-100 p-3 rounded-lg overflow-x-auto my-2 text-xs;
  }
