...
<div class="absolute bottom-0 left-0 right-0 p-4">
  ...
  {#if filteredMentions.length > 0}
    <div class="mb-1 ml-1 bg-white rounded-lg border border-slate-200 shadow-lg py-1 max-h-40 overflow-y-auto">
      {#each filteredMentions as mention, i}
        <button
          class="w-full text-left px-3 py-1.5 text-sm flex items-center gap-2 hover:bg-slate-100
        ...
        >
          ...
          {:else}
            <span class="w-4 h-4 rounded-full bg-teal-100 text-teal-700 text-[10px] font-bold flex items-center justify-center shrink-0">
              {mention.name[0]?.toUpperCase()}
            </span>
          {/if}
          <span class="text-slate-700">{mention.name}</span>
        </button>
      {/each}
    </div>
  {/if}
  ...
  <div class="flex items-end gap-1 bg-white rounded-2xl px-3 py-2 border border-slate-200 shadow-sm
    {hasRoolMention ? 'border-violet-400 ring-1 ring-violet-200' : 'focus-within:border-teal-400'}
    transition-colors">
    <textarea
      bind:this={textarea}
