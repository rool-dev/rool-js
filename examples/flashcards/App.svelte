...
{#snippet topicList(showSelected: boolean)}
  {#if collectionsLoading}
    <p class="text-sm text-slate-400 dark:text-neutral-500 px-3 py-2">Loading...</p>
  {:else if topics.length === 0}
    <p class="text-sm text-slate-400 dark:text-neutral-500 px-3 py-2">No topics yet</p>
  {:else}
...
{#snippet newTopicForm(bgClass: string)}
  {#if isGenerating && showNewTopicForm}
    <div class="mt-4 p-4 text-center">
      ...
      <p class="text-sm text-slate-500 dark:text-neutral-400 mt-2">Creating topic...</p>
    </div>
  {:else if showNewTopicForm}
    <div class="mt-4 p-3 {bgClass} rounded-lg">
      <input
        type="text"
        class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-neutral-600 rounded-lg bg-white dark:bg-neutral-800 text-slate-800 dark:text-neutral-100 placeholder:text-slate-400 dark:placeholder:text-neutral-500 focus:ring-2 focus:ring-violet-500 focus:border-violet-500 focus:outline-none"
      ...
      <div class="flex gap-2 mt-2">
        ...
        <button
          class="px-3 py-1.5 text-sm text-slate-600 dark:text-neutral-300 hover:bg-slate-200 dark:hover:bg-neutral-700 rounded-lg"
  ...
  {:else}
    <button
      class="mt-4 w-full px-3 py-2 text-sm font-medium text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-900/30 rounded-lg hover:bg-violet-100 dark:hover:bg-violet-900/50 transition-colors"
...
<!-- Main UI -->
<div class="h-full bg-slate-50 dark:bg-neutral-950 flex overflow-hidden">
  <!-- Desktop sidebar -->
  <aside class="hidden md:flex md:w-64 bg-white dark:bg-neutral-900 border-r border-slate-200 dark:border-neutral-700 p-4 flex-col">
    <h2 class="text-sm font-semibold text-slate-500 dark:text-neutral-400 uppercase tracking-wider mb-3">Topics</h2>
    <div class="flex-1 space-y-1 overflow-y-auto">
      {@render topicList(true)}
    </div>
    {@render newTopicForm('bg-slate-50 dark:bg-neutral-800')}
  </aside>
    ...
    {#if selectedTopicId}
      ...
      <div class="mb-4 flex items-center justify-between gap-2">
        <div class="flex items-center gap-2 min-w-0">
          <button
            class="md:hidden p-1.5 -ml-1.5 text-slate-500 dark:text-neutral-400 hover:text-slate-700 dark:hover:text-neutral-200"
      ...
      {#if isGenerating}
        <div class="flex-1 flex items-center justify-center">
          <div class="text-center">
            ...
            <p class="text-slate-500 dark:text-neutral-400">Generating cards...</p>
          </div>
        </div>
      {:else if currentCard}
      ...
      {:else}
        <div class="flex-1 flex flex-col items-center justify-center text-center">
          <div class="w-16 h-16 bg-violet-100 dark:bg-violet-900/40 rounded-2xl flex items-center justify-center mb-6">
            <Icon icon="mdi:check-circle-outline" class="w-8 h-8 text-violet-500 dark:text-violet-400" />
          </div>
          <h3 class="text-lg font-semibold text-slate-700 dark:text-neutral-100 mb-2">All caught up!</h3>
          <p class="text-slate-500 dark:text-neutral-400 text-sm mb-6">No cards are due right now.</p>
          <button
    ...
    {:else}
      ...
      <div class="flex-1 flex flex-col">
        ...
        <div class="md:hidden flex-1 flex flex-col">
          ...
          {@render newTopicForm('bg-slate-100 dark:bg-neutral-800')}
        </div>

        <!-- Desktop: placeholder -->
        <div class="hidden md:flex flex-1 flex-col items-center justify-center text-center">
          <div class="w-16 h-16 bg-slate-100 dark:bg-neutral-800 rounded-2xl flex items-center justify-center mb-6">
            <Icon icon="mdi:cards-outline" class="w-8 h-8 text-slate-400 dark:text-neutral-500" />
          </div>
          <h2 class="text-lg font-semibold text-slate-700 dark:text-neutral-100 mb-2">Select a topic</h2>
          <p class="text-slate-500 dark:text-neutral-400 text-sm">Choose a topic from the sidebar or create a new one</p>
        </div>
      </div>
    {/if}
  </div>
