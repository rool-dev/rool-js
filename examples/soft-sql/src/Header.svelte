...
<header class="bg-white border-b border-slate-200 px-4 py-3">
  <div class="max-w-6xl mx-auto flex items-center gap-3">
    ...
    <div class="flex items-center gap-2 shrink-0">
      <div class="w-8 h-8 bg-linear-to-br from-emerald-400 to-cyan-500 rounded-lg flex items-center justify-center">
        <Icon icon="mdi:console" class="w-4 h-4 text-white" />
      </div>
      <span class="font-semibold text-slate-800 hidden sm:block">Soft SQL</span>
    </div>
    ...
    <div class="flex-1 min-w-0 relative" data-dropdown>
      ...
      {:else if showNewSpaceForm}
        <div class="flex items-center gap-2">
          <input
            type="text"
            class="flex-1 min-w-0 px-3 py-1.5 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:outline-none"
          ...
          />
          <button
            class="px-3 py-1.5 text-sm font-medium text-white bg-emerald-500 rounded-lg hover:bg-emerald-600 disabled:opacity-50 shrink-0"
            onclick={createSpace}
        ...
        {#if showSpaceDropdown}
          <div class="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
            {#each rool.spaces ?? [] as s}
              <button
                class="w-full text-left px-3 py-2 text-sm hover:bg-slate-50 flex items-center gap-2 {s.id === currentSpace?.id ? 'bg-emerald-50 text-emerald-700' : 'text-slate-700'}"
