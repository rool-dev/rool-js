...
export const testCase: TestCase = {
  ...
  async run(space) {
    // Import the archive
    await space.importArchive(archive);
    ...
    // Local media URL should have been uploaded and rewritten to a full URL
    expect(enki!.image_url).to.be.a('string');
    expect(enki!.image_url).to.match(/^https?:\/\//); // Should be a full URL now
