...
function getLatestVersion() {
  try {
    const tags = execSync(
      'git tag --list "v*" --merged HEAD --sort=-version:refname',
...
if (latest) {
  ...
  if (newVal <= latestVal) {
    console.error(
      `Error: Version ${version} is not greater than latest release ${latest.tag.slice(1)}`,
    );
    process.exit(1);
  }
  ...
  if (major === latest.major && minor === latest.minor) {
    ...
    if (patch !== latest.patch + 1) {
      console.error(
        `Error: Patch version gap — expected ${latest.major}.${latest.minor}.${latest.patch + 1}, got ${version}`,
      );
      process.exit(1);
    }
    ...
    if (minor !== latest.minor + 1) {
      console.error(
        `Error: Minor version gap — expected ${latest.major}.${latest.minor + 1}.0, got ${version}`,
      );
      process.exit(1);
    }
    if (patch !== 0) {
      console.error(
        `Error: Patch should be 0 when bumping minor — expected ${major}.${minor}.0, got ${version}`,
      );
      process.exit(1);
    }
  } else {
    ...
    if (major !== latest.major + 1) {
      console.error(
        `Error: Major version gap — expected ${latest.major + 1}.0.0, got ${version}`,
      );
      process.exit(1);
    }
    if (minor !== 0 || patch !== 0) {
      console.error(
        `Error: Minor and patch should be 0 when bumping major — expected ${major}.0.0, got ${version}`,
      );
      process.exit(1);
    }
  }
